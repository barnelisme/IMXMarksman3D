...
public class BlueCarManager : MonoBehaviour
{
    public List<GameObject> movePoints = new List<GameObject>();
    public Material blueCarMaterial;
    ...
    void Update()
    {
        ...
        if (!StaticVariableManager.blueCarMoveComplete && StaticVariableManager.isStopTraining == false && countDownStart.start_training == true && StaticVariableManager.flickerSet == false)
        {
            ...
            if (Vector3.Distance(transform.position, currentMovePoint.position) < 0.1f)
            {
                ...
                currentMovePoint = movePoints[currentPointIndex].transform;
                moveSteps++;
                StaticVariableManager.blueCarCompletedPoints++;
                if (StaticVariableManager.blueCarCompletedPoints >= movePoints.Count)
                {
                    StaticVariableManager.isStopTraining = true;
                }
                print(StaticVariableManager.blueCarCompletedPoints);
            ...
            else
            {
                ...
                {
                    moveSteps = StaticVariableManager.blueCarMoveSteps;
                    currentPointIndex = (currentPointIndex + 1) % movePoints.Count;
                    currentMovePoint = movePoints[currentPointIndex].transform;
                ...
                if (moveSteps >= StaticVariableManager.blueCarMoveSteps)
                {
                    StaticVariableManager.blueCarMoveComplete = true;
                    moveSteps = 0;
                }
                else
                {
        ...
        else if (!StaticVariableManager.blueCarMoveComplete && StaticVariableManager.isStopTraining == false && countDownStart.start_training == true && StaticVariableManager.flickerSet == true)
        {
            // Check if we reached the current movePoint
            if (Vector3.Distance(transform.position, currentMovePoint.position) < 0.1f)
            {
                ...
                moveSteps++;
                StaticVariableManager.blueCarCompletedPoints--;
                if (StaticVariableManager.blueCarCompletedPoints >= movePoints.Count + 1)
                {
                    StaticVariableManager.isStopTraining = true;
                }
                //print(StaticVariableManager.blueCarCompletedPoints);
        ...
        if (countDownStart.start_training == true && StaticVariableManager.flickerSet == true )
        {
            if (flickerUpTimer <= 0)
            {
                ...
                {
                    blueCarMaterial.color = startColor;
                    flickerDownTimer -= Time.deltaTime;
                }
            }
            else
            {
                blueCarMaterial.color = Color.black;
        ...
        {
            blueCarMaterial.color = Color.blue;
        }
